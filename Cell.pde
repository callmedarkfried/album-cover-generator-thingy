...
public class Cell implements BoardElement {
  ...
  Cell(color c, PVector p, Bot[] b, int boardSizeX, int boardSizeY) {
    ...
    //PVector hbpos = euclidToExponential(scale(offsetPos, 0.001), -1);
    //float hyperDist = sqrt(hbpos.x*hbpos.x + hbpos.y*hbpos.y);
    //if(hbpos.y % 0.1 > 0.095 || hbpos.x % 0.1 > 0.095 || hbpos.x % 0.1 < .005) {
    //  scale = 0;
    //} else {
    //  scale = 100;
    //}
    ...
    //PVector squaredComplex = complexRoot(scale(offsetPos,0.1), 5, 0);
    //squaredComplex = new PVector(squaredComplex.x, squaredComplex.y);
    //float squaredComplexDist = sqrt(squaredComplex.x*squaredComplex.x + squaredComplex.y*squaredComplex.y);
    //if (abs(squaredComplex.x) % 1 < 0.05 || abs(squaredComplex.y) % 1 < 0.05) {
    //  scale = 0;
    //} else {
    //  scale = 100;
    //}
    //scale = 1*log(squaredComplexDist+1);caleidoscope(4.5, p, 0.01, 0.5, 1)
    
    scale = 1/(5*test(30, complexTransformationRoot(scale(offsetPos,0.1)), 0.5, 7.2, 4));
    
    //scale = 1/(5*test(30, offsetPos, 0.5, caleidoscope(4.5, p, 0.01, 0.5, 1), 1));
    
    //scale = 1/(5*test(20, cartesianToHyperbolic(scale(offsetPos,.5)), 0.5, 7.2, 1));
    //scale = (2*test(40, cartesianToParabolic(scale(offsetPos,3)), 0.5, 7.2, 1));
    
    //scale = (10*test(20, cartesianToPolar(offsetPos), 0.5, 7.2, 1));
    
    //scale = test(1, euclidToExponential(scale(offsetPos, 0.002), -1), 0.5, 7.2, 1);
    
    // scale = distFromOff / 10 / hyperDist;

    //scale = 0.1 * (hydrogenWaveFunction(offsetPos.x / 500, offsetPos.y / 500, -10, 1,1,1));
  ...
  private float test (float scale, PVector in, float a, float b, int i){
    PVector c = new PVector(abs(in.x/scale), abs(in.y/scale));
    return ((float)i) * abs((cos(b*pow(c.x, a)) + cos(b*pow(c.y, a)))/abs(c.x*c.y));
  }

  private PVector complexRoot(PVector in, int root, int k) {
    in.x = abs(in.x);
    in.y = abs(in.y);
    Complex c = new Complex(in.x, in.y);
    return new PVector(c.nthRoot(root, k).real, c.nthRoot(root, k).img);
  }
  private PVector complexExponentiation(PVector in, int exponent) {
    in.x = abs(in.x);
    in.y = abs(in.y);
    Complex c = new Complex(in.x, in.y);
    Complex r = c;
    ...
    return new PVector(r.real, r.img);
  }
  
  private float hydrogenWaveFunction(float r, float a0) {


    return 1 / (sqrt(PI) * pow(a0, 3/2)) * exp(-r/a0);
  }

  private PVector complexTransformationRoot(PVector in) {
    in.x = abs(in.x);
    in.y = abs(in.y);
    return new PVector(abs(in.x * in.x - in.y * in.y), abs(2*in.x*in.y));
  }
  
  private PVector complexTransformationSquare(PVector in) {
    in.x = abs(in.x);
    in.y = abs(in.y);
    float d = sqrt(2 * (in.x + sqrt(in.x*in.x + in.y*in.y)));
    float x = (in.x + sqrt(in.x*in.x + in.y*in.y)) / d;
    float y = in.y / d;
    return new PVector(x, y);
  }
  
  private PVector complexTransformationCbrt(PVector in) {
    in.x = abs(in.x);
    in.y = abs(in.y);
    return new PVector(abs(in.x*in.x*in.x - 3*in.x*in.y*in.y), abs(3*in.x*in.x*in.y));
  }

  private float hydrogenWaveFunction(float x, float y, float z, float l, float m, float n) {
    if (sign(x) != sign(y)) {
      x = abs(x);
      y = abs(y);
    }
  ...
  x = \sqrt{\left|x^{2}-y^{2}\right|}
  y = \sqrt{\left|\left(x\cos n-y\sin n\right)^{2}-\left(x\sin n+y\cos n\right)^{2}\right|}
  
  */
  //
  private PVector euclidToExponential(PVector in, float fac) {
    PVector out = new PVector(0, 0);
